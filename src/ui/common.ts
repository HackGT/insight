...
async function sendRequest(
  ...
  const response: APIResponse = await fetch(url, options).then(response => response.json());
  if (!response.success) {
...
function setUpHandlers(classname: string, handler: (dataset: Dataset) => Promise<void>) {
  const buttons = document.getElementsByClassName(classname) as HTMLCollectionOf<HTMLButtonElement>;
  for (let i = 0; i < buttons.length; i++) {
    buttons[i].addEventListener(
      "click",
      asyncHandler(async button => {
